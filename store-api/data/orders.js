...
import { getCart, cartToDTO } from "../data/carts.js";

const apiUrl = process.env.VITE_API_URL;
const apiKey = process.env.VITE_API_KEY;

console.log(apiUrl); // örnek: https://api.example.com
console.log(apiKey); // örnek: abcdefg12345
dotenv.config();
...
async function add(order) {
    ...
    // calculate Total
    const total = cart?.cartItems.reduce((toplam, item) => toplam + item.product.price * item.product.quantity, 0);
    ...
    // clear cart items
    const index = data.carts.findIndex((p) => p.customerId === newOrder.customerId);
